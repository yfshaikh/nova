...
Simple3DObject createFloorGrid() {
    Simple3DObject grid(sl::Translation(0, 0, 0), true);
    const float to_f = 1.f / 255.f;
    const sl::float3 clr(120 * to_f, 120 * to_f, 120 * to_f);
    for (int i = -10; i <= 10; ++i) {
        grid.addPoint(sl::float3(-10.f, 0.f, (float)i), clr);
        grid.addPoint(sl::float3(10.f, 0.f, (float)i), clr);
        grid.addPoint(sl::float3((float)i, 0.f, -10.f), clr);
        grid.addPoint(sl::float3((float)i, 0.f, 10.f), clr);
    }
    grid.setDrawingType(GL_LINES);
    return grid;
}

GLenum GLViewer::initMulti(int argc, char** argv, sl::Resolution image_size) {
    ...
    glutCreateWindow("Nova 4x ZED fused cloud (GPU-resident)");

    GLenum err = glewInit();
    if (GLEW_OK != err)
        return err;

    pc_resolution_ = image_size;

    // Compile and create the shader
    shader_.set(VERTEX_SHADER, FRAGMENT_SHADER);
    shMVPMatrixLoc_ = glGetUniformLocation(shader_.getProgramId(), "u_mvpMatrix");

    // Orbit camera — rig coords are meters (X-right, Y-down, Z-forward)
    camera_ = CameraGL(sl::Translation(0.f, -1.5f, 6.f), sl::Translation(0.f, 0.f, -1.f));

    sl::Rotation rot;
    rot.setEulerAngles(sl::float3(15.f, 0.f, 0.f), false);
    camera_.setRotation(rot);
    camera_.setProjection(70, 70, 0.05f, 200.f);

    floor_grid_ = createFloorGrid();
    floor_grid_.pushToGPU();
...
void GLViewer::setFpsText(float fps) {
    display_fps_ = fps;
    char title[128];
    snprintf(title, sizeof(title), "Nova 4x ZED fused cloud — %.1f fps", fps);
...
void GLViewer::updatePointCloud(unsigned serial, sl::Mat& matXYZRGBA) {
    auto it = point_clouds_.find(serial);
    if (it == point_clouds_.end())
        return;
    it->second.mutexData.lock();
    it->second.pushNewPC(matXYZRGBA);
    it->second.mutexData.unlock();
}

void GLViewer::update() {
    if (keyStates_['q'] == KEY_STATE::UP || keyStates_['Q'] == KEY_STATE::UP || keyStates_[27] == KEY_STATE::UP) {
        for (auto& kv : point_clouds_)
            kv.second.close();
        currentInstance_->exit();
        return;
    }
    ...
    // Translate camera with mouse (meter-scale rig)
    if (mouseButton_[MOUSE_BUTTON::RIGHT]) {
        camera_.translate(camera_.getUp() * (float)mouseMotion_[1] * MOUSE_T_SENSITIVITY * 0.05f);
        camera_.translate(camera_.getRight() * (float)mouseMotion_[0] * MOUSE_T_SENSITIVITY * 0.05f);
    }

    // Zoom with mouse wheel
    if (mouseWheelPosition_ != 0) {
        if (mouseWheelPosition_ > 0) {
            camera_.translate(camera_.getForward() * MOUSE_UZ_SENSITIVITY * 0.05f * -1);
        } else if (mouseWheelPosition_ < 0) {
            camera_.translate(camera_.getForward() * MOUSE_UZ_SENSITIVITY * 0.05f);
        }
    }

    // D2D copy from ZED GPU mats into GL/CUDA interop buffers
    for (auto& kv : point_clouds_) {
        kv.second.mutexData.lock();
        kv.second.update();
        kv.second.mutexData.unlock();
    }
    camera_.update();
    clearInputs();
}
...
void GLViewer::draw() {
    ...
    glPointSize(2.f);
    for (auto& kv : point_clouds_) {
        const unsigned serial = kv.first;
        auto pose_it = cam_to_rig_.find(serial);
        if (pose_it == cam_to_rig_.end())
            continue;
        const sl::Transform mvp = vpMatrix * pose_it->second;
        kv.second.draw(mvp);
    }
}
